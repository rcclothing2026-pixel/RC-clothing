<header data-header
    ...
    <div class="mx-auto flex max-w-7xl items-center gap-4 px-4 py-2.5 sm:px-6 lg:gap-6 lg:py-2.5">
        ...
        <button type="button" @click="navOpen = true" :aria-expanded="navOpen" aria-label="منو" class="grid h-9 w-9 shrink-0 place-items-center rounded-lg text-brand-700 ring-1 ring-brand-200 lg:hidden">
        ...
        <nav class="hidden items-center gap-1 lg:order-1 lg:flex lg:flex-1 lg:justify-start">
            @if ($headerMenu->isNotEmpty())
                ...
                @endforeach
            @else
                <a href="{{ route('shop.index') }}" class="rounded-lg px-3 py-2 text-sm font-medium text-brand-700 transition hover:bg-brand-100">فروشگاه</a>
        ...
        <div class="flex items-center gap-0.5 ms-auto md:ms-0 lg:ms-0">
            <a href="{{ auth()->check() ? route('account.index') : route('login') }}"
               class="rounded-lg p-2 text-brand-600 hover:bg-brand-100 hover:text-brand-900" aria-label="حساب کاربری">
            ...
            <a href="{{ route('wishlist.index') }}"
               class="rounded-lg p-2 text-brand-600 hover:bg-brand-100 hover:text-brand-900" aria-label="علاقه‌مندی‌ها">
            ...
            @endauth
            <a href="{{ route('cart.index') }}" class="relative rounded-lg p-2 text-brand-600 hover:bg-brand-100 hover:text-brand-900" aria-label="سبد خرید">
                ...
                <span data-cart-count
                      class="absolute -top-0.5 -end-0.5 grid h-4 w-4 place-items-center rounded-full bg-accent-600 text-[10px] font-bold text-white fa-num {{ ($cartCount ?? 0) > 0 ? '' : 'hidden' }}">{{ \App\Support\Money::toPersianDigits((string) ($cartCount ?? 0)) }}</span>
            </a>
        </div>
        </div>{{-- /right group --}}
    </div>
    ...
    <div class="lg:hidden">
        ...
        <div x-show="navOpen" x-cloak
             ...
             role="dialog" aria-modal="true" aria-label="منوی موبایل">
            ...
            <div class="flex items-center justify-between border-b border-brand-100 px-4 py-3">
                ...
                <button type="button" @click="navOpen = false" aria-label="بستن"
            ...
            <nav class="min-h-0 flex-1 overflow-y-auto px-3 py-3">
                @if ($headerMenu->isNotEmpty())
                        ...
                        @if ($item->children->isNotEmpty())
                            <div x-data="{ open: false }" class="border-b border-brand-50">
                                <div class="flex items-center">
                                    ...
                                    <button type="button" @click="open = !open" :aria-expanded="open" aria-label="زیرمجموعه‌ها"
                    ...
                    @endforeach
                @else
                    <a href="{{ route('shop.index') }}" @click="navOpen = false" class="block rounded-lg px-3 py-3 text-sm font-semibold text-brand-800 transition hover:bg-brand-50">فروشگاه</a>
