    ...
    <aside x-show="visible"
        ...
        <div class="border-t border-brand-100 bg-brand-50/60 px-5 py-4">
            <div class="mb-3 flex items-center justify-between text-sm">
                <span class="text-brand-500">{{ __('Total') }}</span>
                <span class="text-lg font-bold text-brand-900 fa-num" x-text="state.subtotal"></span>
            </div>
            <div class="flex gap-2">
                <a :href="state.checkout_url"
                   class="flex-1 rounded-full bg-brand-900 py-3 text-center text-sm font-semibold text-white transition hover:bg-brand-800">{{ __('Checkout') }}</a>
                <a :href="state.cart_url"
                   class="rounded-full px-5 py-3 text-center text-sm font-semibold text-brand-700 ring-1 ring-brand-200 transition hover:bg-white">{{ __('View Bag') }}</a>
            </div>
            <button type="button" @click="close()"
                    class="mt-2 w-full py-2 text-xs text-brand-400 hover:text-brand-600">{{ __('Continue Shopping') }}</button>
        </div>
    </aside>
</div>
