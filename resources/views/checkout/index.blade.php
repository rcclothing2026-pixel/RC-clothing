...
@section('title', 'تسویه‌حساب | چیاکو')
    ...
    <div class="mx-auto max-w-5xl px-4 py-10 sm:px-6" x-data="{ submitting: false }">
        ...
        <div class="mb-10 flex items-center justify-center gap-0 text-xs sm:text-sm">
            <div class="flex items-center">
                <span class="flex h-8 w-8 items-center justify-center rounded-full bg-brand-900 text-white">۱</span>
                <span class="mr-2 font-medium text-brand-900">سبد خرید</span>
            </div>
            <div class="mx-3 h-px w-10 bg-brand-300 sm:w-16"></div>
            <div class="flex items-center">
                <span class="flex h-8 w-8 items-center justify-center rounded-full bg-brand-900 text-white">۲</span>
                <span class="mr-2 font-medium text-brand-900">تسویه‌حساب</span>
            </div>
            <div class="mx-3 h-px w-10 bg-brand-300 sm:w-16"></div>
            <div class="flex items-center">
                <span class="flex h-8 w-8 items-center justify-center rounded-full bg-brand-200 text-brand-500">۳</span>
                <span class="mr-2 text-brand-400">تأیید سفارش</span>
            </div>
        </div>
        ...
        <form action="{{ route('checkout.place') }}" method="POST" class="grid gap-8 lg:grid-cols-[1fr_340px]" data-checkout
            ...
            <div class="space-y-6">
                ...
                <section class="rounded-card bg-white p-6 ring-1 ring-brand-100" x-data="{ mode: '{{ $addresses->isNotEmpty() ? 'saved' : 'new' }}' }">
                    <h2 class="mb-4 text-base font-bold text-brand-900">آدرس تحویل</h2>
                    ...
                    @if ($addresses->isNotEmpty())
                        ...
                        <button type="button" x-on:click="mode = (mode === 'new' ? 'saved' : 'new')"
                                ...
                                x-text="mode === 'new' ? 'انتخاب آدرس ذخیره‌شده' : '+ آدرس جدید'"></button>
                    @endif
                    ...
                    <div x-show="mode === 'new'" x-transition x-cloak class="mt-3 grid grid-cols-2 gap-2"
                        ...
                        <input name="recipient_name" value="{{ old('recipient_name', $user->name) }}" :disabled="mode !== 'new'"
                               placeholder="نام گیرنده" class="rounded-lg border border-brand-200 px-3 py-2 text-sm" required>
                        <input name="phone" value="{{ old('phone', $user->phone) }}" dir="ltr" inputmode="numeric" :disabled="mode !== 'new'"
                               placeholder="موبایل گیرنده" class="rounded-lg border border-brand-200 px-3 py-2 text-sm" required>
                        ...
                        <input name="postal_code" value="{{ old('postal_code') }}" dir="ltr" inputmode="numeric" :disabled="mode !== 'new'"
                               placeholder="کد پستی (اختیاری)" class="col-span-2 rounded-lg border border-brand-200 px-3 py-2 text-sm">
                        <textarea name="line" placeholder="نشانی کامل" class="col-span-2 rounded-lg border border-brand-200 px-3 py-2 text-sm" required :disabled="mode !== 'new'">{{ old('line', $defaultAddress) }}</textarea>
                    </div>
                ...
                <section class="rounded-card bg-white p-6 ring-1 ring-brand-100">
                    <h2 class="mb-4 text-base font-bold text-brand-900">یادداشت</h2>
                    <textarea name="customer_note" placeholder="یادداشت خود را برای سفارش بنویسید (اختیاری)" rows="3"
                ...
                @if (($site['site.giftwrap_enabled'] ?? null))
                    ...
                    <section class="rounded-card bg-white p-6 ring-1 ring-brand-100"
                        ...
                        <h2 class="mb-2 text-base font-bold text-brand-900">🎁 بسته‌بندی کادویی</h2>
                        @if ($wrapLabel)
                        ...
                        <label class="flex cursor-pointer items-start gap-3 rounded-lg border border-brand-200 p-3 text-sm has-[:checked]:border-brand-800 has-[:checked]:bg-brand-50">
                            ...
                            <span class="flex-1">
                                <span class="font-medium text-brand-800">سفارش با بسته‌بندی کادویی ارسال شود</span>
                                <span class="mt-1 block text-xs text-brand-500" data-giftwrap-price-hint>—</span>
                            </span>
                        </label>
                        <div x-show="wrap" x-cloak class="mt-3">
                            <label class="mb-1 block text-sm font-medium text-brand-700">متن کارت پیام (روی کارت چاپ می‌شود — اختیاری)</label>
                            <textarea name="gift_message" rows="3" maxlength="500" placeholder="مثلاً: تولدت مبارک عزیزم..."
                ...
                {{-- Shipping --}}
                <section class="rounded-card bg-white p-6 ring-1 ring-brand-100">
                    <h2 class="mb-4 text-base font-bold text-brand-900">شیوه ارسال</h2>
                    <div class="space-y-2">
                            ...
                            <label class="flex cursor-pointer items-center justify-between gap-3 rounded-lg border border-brand-200 p-3 text-sm has-[:checked]:border-brand-800 has-[:checked]:bg-brand-50">
                                <span class="flex items-center gap-3">
                                    ...
                                    <span>
                                        ...
                                        @if ($method->cost_on_delivery)
                                            <span class="ms-2 rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700">پس‌کرایه</span>
                                        @endif
                                ...
                                <span class="font-semibold text-brand-700">
                                    @if ($method->cost_on_delivery)
                                        پرداخت به مأمور
                                    @else
                                        {{ $method->costFor($subtotal) > 0 ? \App\Support\Money::toman($method->costFor($subtotal)) : 'رایگان' }}
                                    @endif
                                </span>
                            </label>
                        @endforeach
                ...
                {{-- Payment method --}}
                <section class="rounded-card bg-white p-6 ring-1 ring-brand-100">
                    <h2 class="mb-4 text-base font-bold text-brand-900">روش پرداخت</h2>
                    @if ($paymentMethods->isEmpty())
                        <p class="text-sm text-red-500">هیچ درگاه پرداختی فعال نیست.</p>
                    @else
                        <div class="space-y-2">
            ...
            <aside class="h-fit rounded-card bg-white p-6 ring-1 ring-brand-100">
                <h2 class="mb-4 text-base font-bold text-brand-900">خلاصه سفارش</h2>
                <div class="max-h-48 space-y-2 overflow-auto border-b border-brand-100 pb-3">
                    @foreach ($lines as $line)
                        <div class="flex items-center justify-between text-xs text-brand-600">
                            <span class="line-clamp-1">{{ $line['variant']->product->name }} ×<span class="fa-num">{{ \App\Support\Money::toPersianDigits((string) $line['quantity']) }}</span></span>
                            <span>{{ \App\Support\Money::toman($line['line_total'], false) }}</span>
                        </div>
                    @endforeach
                </div>
                <div class="space-y-2 py-3 text-sm" aria-live="polite">
                    <div class="flex justify-between"><span class="text-brand-500">جمع کالاها</span><span>{{ \App\Support\Money::toman($subtotal) }}</span></div>
                    @if ($coupon)
                        <div class="flex justify-between"><span class="text-green-600">کد «{{ $coupon->code }}»</span><span class="text-green-600">{{ $discount > 0 ? '−'.\App\Support\Money::toman($discount) : 'ارسال رایگان' }}</span></div>
                    @endif
                    @foreach ($ruleDiscounts as $rd)
                        @php $rda = (int) ($rd['amount'] ?? 0); @endphp
                        @if ($rda > 0)
                            <div class="flex justify-between"><span class="text-green-600">{{ $rd['rule_name'] ?? 'تخفیف هوشمند' }}</span><span class="text-green-600">−{{ \App\Support\Money::toman($rda) }}</span></div>
                        @elseif ($rda < 0)
                            <div class="flex justify-between"><span class="text-amber-600">{{ $rd['rule_name'] ?? 'هزینه اضافه' }}</span><span class="text-amber-600">+{{ \App\Support\Money::toman(abs($rda)) }}</span></div>
                        @endif
                    @endforeach
                    @if ($loyalty && $loyalty['points'] >= $loyalty['min_redeem'] && $loyalty['toman_per_point'] > 0)
                        <div class="border-t border-brand-100 pt-2">
                            <label class="flex items-center justify-between text-sm">
                                <span class="text-brand-600">امتیاز باشگاه (موجودی <span class="fa-num">{{ \App\Support\Money::toPersianDigits((string) $loyalty['points']) }}</span>)</span>
                            </label>
                            <div class="mt-1 flex items-center gap-2">
                                <input type="number" name="redeem_points" min="0" max="{{ $loyalty['points'] }}" value="0" data-redeem inputmode="numeric"
                                       class="w-24 rounded-lg border border-brand-200 px-2 py-1.5 text-sm fa-num" placeholder="امتیاز">
                                <span class="text-xs text-brand-400">معادل <span data-redeem-value>۰</span> تومان</span>
                            </div>
                        </div>
                        <div class="flex justify-between" data-loyalty-row style="display:none"><span class="text-green-600">تخفیف امتیاز</span><span class="text-green-600" data-loyalty-display>—</span></div>
                    @endif
                    <div class="flex justify-between"><span class="text-brand-500">هزینه ارسال</span><span data-shipping-display>—</span></div>
                    @if (($site['site.giftwrap_enabled'] ?? null))
                        <div class="flex justify-between" data-giftwrap-row style="display:none"><span class="text-brand-500">بسته‌بندی کادویی</span><span data-giftwrap-display>—</span></div>
                    @endif
                    @if ($giftCard)
                        <div class="flex justify-between" data-gift-row style="display:none"><span class="text-green-600">کارت هدیه</span><span class="text-green-600" data-gift-display>—</span></div>
                    @endif
                    <div class="flex justify-between border-t border-brand-100 pt-2 text-base font-bold"><span>قابل پرداخت</span><span class="text-brand-900" data-total-display>—</span></div>
                </div>
                <button type="submit" :disabled="submitting || {{ $paymentMethods->isEmpty() ? 'true' : 'false' }}"
                    ...
                    <span x-show="!submitting">پرداخت و ثبت سفارش</span>
                    ...
                    <span x-show="submitting">در حال اتصال به درگاه...</span>
                </button>
                <div class="mt-3 flex items-center justify-center gap-2">
                    <svg aria-hidden="true" class="h-6 w-auto text-brand-300" viewBox="0 0 40 24" fill="currentColor"><rect width="40" height="24" rx="4"/><text x="20" y="16" text-anchor="middle" fill="white" font-size="10" font-weight="bold">شاپرک</text></svg>
                    <svg aria-hidden="true" class="h-6 w-auto text-brand-300" viewBox="0 0 40 24" fill="currentColor"><rect width="40" height="24" rx="4"/><text x="20" y="16" text-anchor="middle" fill="white" font-size="10" font-weight="bold">سداد</text></svg>
                    <svg aria-hidden="true" class="h-6 w-auto text-brand-300" viewBox="0 0 40 24" fill="currentColor"><rect width="40" height="24" rx="4"/><text x="20" y="16" text-anchor="middle" fill="white" font-size="10" font-weight="bold">ملت</text></svg>
                    <svg aria-hidden="true" class="h-6 w-auto text-brand-300" viewBox="0 0 40 24" fill="currentColor"><rect width="40" height="24" rx="4"/><text x="20" y="16" text-anchor="middle" fill="white" font-size="10" font-weight="bold">پاسارگاد</text></svg>
                </div>
                ...
                <div class="mt-4 grid grid-cols-3 gap-2 border-t border-brand-100 pt-4 text-center">
                    <div class="flex flex-col items-center gap-1">
                        ...
                        <span class="text-[11px] leading-4 text-brand-500">پرداخت امن<br>درگاه بانکی معتبر</span>
                    </div>
                    <div class="flex flex-col items-center gap-1">
                        ...
                        <span class="text-[11px] leading-4 text-brand-500">۷ روز ضمانت<br>بازگشت کالا</span>
                    </div>
                    <div class="flex flex-col items-center gap-1">
                        ...
                        <span class="text-[11px] leading-4 text-brand-500">پشتیبانی<br>پاسخگو</span>
                    </div>
                </div>
            </aside>
        </form>
    ...
    <script>
        (function () {
            ...
            function shippingCost() {
                ...
                // پس‌کرایه — courier collects on delivery; doesn't count toward online total.
            ...
            function paintGiftWrapHint() {
                ...
                if (freeOver > 0 && subtotal >= freeOver) {
                    wrapHint.textContent = 'برای این سفارش رایگان است.';
                } else {
                    wrapHint.textContent = fmt(perItem) + ' × ' + faN(String(items)) + ' قلم = ' + fmt(Math.max(0, perItem * items));
                }
            }
            ...
            function update() {
                ...
                if (shippingEl) shippingEl.textContent = shippingIsCod() ? 'پس‌کرایه' : (cost > 0 ? fmt(cost) : 'رایگان');
                if (wrapRow) wrapRow.style.display = wrapToggle?.checked ? '' : 'none';
                if (wrapEl) wrapEl.textContent = wrap > 0 ? fmt(wrap) : 'رایگان';
            ...
            // Clear, Persian guidance for missing required fields. On a blocked
            // submit the browser fires `invalid` per field: we give each a precise
            // localized message (shown in the native prompt) and highlight every
            // missing field in red so the user sees exactly what to complete.
            const FA_MSG = {
                recipient_name: 'لطفاً نام گیرنده را وارد کنید.',
                phone: 'لطفاً شماره موبایل گیرنده را وارد کنید.',
                province: 'لطفاً استان را انتخاب کنید.',
                city: 'لطفاً شهر را انتخاب کنید.',
                line: 'لطفاً نشانی کامل گیرنده را وارد کنید.',
                address_id: 'لطفاً آدرس تحویل را انتخاب یا وارد کنید.',
                shipping_method_id: 'لطفاً شیوه ارسال را انتخاب کنید.',
                payment_method: 'لطفاً روش پرداخت را انتخاب کنید.',
            };
            const mark = (el, on) => {
            ...
            root.querySelectorAll('[required]').forEach((el) => {
                el.addEventListener('invalid', () => {
                    el.setCustomValidity(FA_MSG[el.name] || 'تکمیل این بخش الزامی است.');
                    mark(el, true);
                });
