...
@section('title', __('Payment Successful') . ' | Racket Club')
...
@section('content')
    <div class="mx-auto max-w-lg px-4 py-16 text-center sm:px-6">
        ...
        <div class="relative rounded-card bg-white p-10 ring-1 ring-brand-100" x-data="{ showItems: false }">
            ...
            <h1 class="text-xl font-bold text-brand-900">{{ __('Your order is confirmed') }}</h1>
            <p class="mt-2 text-sm text-brand-500">{{ __('Order number:') }} <span class="font-semibold text-brand-800 fa-num" dir="ltr">{{ $order->number }}</span></p>
            @if ($order->payment?->ref_id)
                <p class="mt-1 text-xs text-brand-400">{{ __('Payment tracking code:') }} <span class="fa-num" dir="ltr">{{ $order->payment->ref_id }}</span></p>
            @endif
            <p class="mt-4 text-lg font-bold text-brand-900">{{ $order->formattedTotal() }}</p>
            @if ($order->gift_wrap)
                <p class="mt-2 text-xs text-amber-700">{{ __('Your order will ship gift-wrapped.') }}</p>
            @endif
            @if ($order->shipping_cost_on_delivery)
                <p class="mt-2 text-xs text-amber-700">{{ __('Shipping is paid on delivery to the courier.') }}</p>
            @endif
            ...
            <button @click="showItems = !showItems" class="mt-4 flex w-full items-center justify-center gap-1 text-xs text-brand-400 transition hover:text-brand-600">
                <span x-text="showItems ? '{{ __('Hide details') }}' : '{{ __('View order details') }}'"></span>
            ...
            <div x-show="showItems" x-collapse x-cloak class="mt-3 border-t border-brand-100 pt-3 text-left">
                <div class="divide-y divide-brand-50 text-sm">
                    @foreach ($order->items as $item)
                        <div class="flex items-center justify-between py-2">
                            <span class="text-brand-800">{{ $item->name }} ×<span class="fa-num">{{ $item->quantity }}</span></span>
            ...
            <div class="mt-6 border-t border-brand-100 pt-4">
                <p class="mb-2 text-xs text-brand-400">{{ __('Share your purchase with friends') }}</p>
                <div class="flex items-center justify-center gap-2">
                    <a href="https://wa.me/?text={{ urlencode('Order '.$order->number.' from Racket Club') }}" target="_blank" class="rounded-full bg-green-100 p-2 text-green-600 transition hover:bg-green-200" aria-label="{{ __('Share on WhatsApp') }}">
                    ...
                    <a href="https://t.me/share/url?url={{ urlencode(url()->current()) }}&text={{ urlencode('Order '.$order->number.' from Racket Club') }}" target="_blank" class="rounded-full bg-blue-100 p-2 text-blue-600 transition hover:bg-blue-200" aria-label="{{ __('Share on Telegram') }}">
                    ...
                    <button @click="navigator.clipboard.writeText(window.location.href); $el.querySelector('span').textContent='{{ __('Copied') }}'" class="rounded-full bg-brand-100 p-2 text-brand-600 transition hover:bg-brand-200" aria-label="{{ __('Copy link') }}">
            ...
            <div class="mt-6 flex justify-center gap-3">
                <a href="{{ route('account.orders.show', $order) }}" class="rounded-full bg-brand-900 px-6 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-800">{{ __('View Order') }}</a>
                <a href="{{ route('shop.index') }}" class="rounded-full px-6 py-2.5 text-sm font-semibold text-brand-700 ring-1 ring-brand-200 transition hover:bg-brand-50">{{ __('Continue Shopping') }}</a>
            </div>
        </div>
    </div>
@endsection
