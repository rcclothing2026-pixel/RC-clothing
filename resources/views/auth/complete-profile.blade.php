...
@section('title', 'تکمیل حساب | چیاکو')
...
@section('content')
    <div class="mx-auto flex max-w-md flex-col justify-center px-4 py-16">
        <div class="rounded-card bg-white p-8 text-center ring-1 ring-brand-100">
            <h1 class="text-xl font-bold text-brand-900">به چیاکو خوش آمدید 👋</h1>
            <p class="mt-2 text-sm leading-7 text-brand-500">برای تکمیل حساب و تجربه‌ی بهتر خرید، نام و نام خانوادگی خود را وارد کنید.</p>

            <form method="POST" action="{{ route('profile.complete.store') }}" class="mt-6 space-y-4 text-right">
                @csrf
                <div>
                ...
                <div>
                    <label class="mb-1 block text-sm text-brand-600">نام خانوادگی</label>
                ...
                <button class="w-full rounded-lg bg-brand-900 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-800">ادامه</button>
            </form>
        </div>
    </div>
@endsection
