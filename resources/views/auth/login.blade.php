...
@section('title', 'Sign In | Racket Club')
    ...
    <div class="flex min-h-[80vh] items-center justify-center px-4 py-16 sm:px-6">
        <div class="w-full max-w-sm">
            ...
            <div class="mb-8 text-center">
                ...
                <h1 class="mt-4 text-xl font-bold uppercase tracking-wide text-brand-900">WELCOME BACK</h1>
                <p class="mt-1.5 text-sm text-brand-500">Enter your mobile number and we'll send a verification code.</p>
            </div>
            ...
            <div class="rounded-2xl bg-white p-7 shadow-sm ring-1 ring-brand-100">
                ...
                <form action="{{ route('login.otp') }}" method="POST" class="space-y-5">
                    @csrf
                    <div>
                        <label for="phone" class="mb-1.5 block text-sm font-medium text-brand-700">Mobile Number</label>
                ...
                <p class="mt-5 text-center text-xs text-brand-400 leading-6">
                    By signing in, you agree to Racket Club's <a href="{{ route('page', 'terms') }}" class="text-accent-600 hover:underline">Terms &amp; Conditions</a>.
                </p>
            </div>

            <div class="mt-4 text-center">
                <a href="{{ route('shop.index') }}" class="text-sm text-brand-400 transition hover:text-brand-600">Continue as guest — browse the shop &rarr;</a>
            </div>
