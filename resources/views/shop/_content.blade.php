{{-- The shop listing: collection/category hero, filter sidebar + product
     grid + live AJAX filtering, and the scroll-to-top. Extracted from
     shop/index so it can render BOTH as the legacy full page AND as the
     shop_products page-builder block. Needs $products/$categories/$sizes/
     $activeCategory/$activeCollection/$maxPrice from ShopController. --}}
    ...
    @if ($activeCollection)
        <section class="relative overflow-hidden bg-brand-900">
            ...
            <div class="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 sm:py-20">
                <p class="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-white/60"><x-brand-dot class="h-2.5 w-2.5 text-accent-500" />{{ __('Collection') }}</p>
    ...
    @elseif ($activeCategory && $activeCategory->image_path)
        <section class="relative overflow-hidden bg-brand-900">
            ...
            <div class="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 sm:py-18">
                <p class="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-white/60"><x-brand-dot class="h-2.5 w-2.5 text-accent-500" />{{ __('Category') }}</p>
    ...
    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      ...
      <div id="shop-frame">
        ...
        <div class="mb-6 flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-brand-900">
                    {{ $activeCollection?->name ?? $activeCategory?->name ?? __('Shop') }}
                </h1>
                <p class="mt-0.5 text-sm text-brand-500 fa-num">{{ $products->total() }} {{ __('products') }}</p>
            </div>
            ...
            <div class="mb-5 flex flex-wrap gap-2">
                ...
                @if (request('category'))
                    <a href="{{ request()->fullUrlWithQuery(['category' => null, 'page' => null]) }}"
                       class="flex items-center gap-1.5 rounded-full bg-brand-100 px-3 py-1.5 text-xs font-medium text-brand-700 transition hover:bg-brand-200">
                        {{ __('Category') }}: {{ $categories->firstWhere('slug', request('category'))?->name ?? request('category') }}
                        <svg aria-hidden="true" class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12"/></svg>
                    </a>
                @endif
                @if (request('size'))
            ...
            <aside id="filter-drawer"
                ...
                <form action="{{ route('shop.index') }}" method="GET"
                    ...
                    <div class="flex items-center justify-between lg:hidden">
                        <span class="font-bold text-brand-900">{{ __('Filters') }}</span>
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-brand-800">{{ __('Search') }}</label>
                        <div class="relative" data-search-autocomplete>
                            <input type="search" name="q" value="{{ request('q') }}" placeholder="{{ __('Product name...') }}"
                                   autocomplete="off"
                    ...
                    <div>
                        <span class="mb-2 block text-sm font-semibold text-brand-800">{{ __('Category') }}</span>
                        <div class="space-y-1.5">
                            <label class="flex cursor-pointer items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm text-brand-600 transition hover:bg-brand-50">
                                <input type="radio" name="category" value="" @checked(! request('category')) onchange="this.form.requestSubmit()" class="accent-brand-900"> {{ __('All') }}
                            </label>
                    ...
                    <div>
                        <span class="mb-2 block text-sm font-semibold text-brand-800">{{ __('Size') }}</span>
                    ...
                    <div x-data="{
                        ...
                        <span class="mb-2 block text-sm font-semibold text-brand-800">{{ __('Price Range (Toman)') }}</span>
                    ...
                    <div class="flex gap-2">
                        <button type="submit"
                                class="flex-1 rounded-xl bg-brand-900 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-800 active:scale-[0.98]">{{ __('Apply Filters') }}</button>
                        <a href="{{ route('shop.index') }}"
                           class="grid place-items-center rounded-xl px-3 text-sm text-brand-500 ring-1 ring-brand-200 transition hover:bg-brand-50">{{ __('Clear') }}</a>
                    </div>
                </form>
            </aside>
            ...
            <div>
                ...
                <div class="mb-5 flex items-center justify-between gap-3">
                    <div class="flex items-center gap-2">
                        <span class="text-xs text-brand-400">{{ __('Sort') }}:</span>
                        <form action="{{ route('shop.index') }}" method="GET" id="sort-form">
                            ...
                            <select name="sort" onchange="document.getElementById('sort-form').requestSubmit()"
                                ...
                                <option value="popular" @selected(request('sort') === 'popular')>{{ __('Most Viewed') }}</option>
                                <option value="bestseller" @selected(request('sort') === 'bestseller')>{{ __('Best Selling') }}</option>
                            </select>
                        </form>
                    </div>
                    <div class="flex items-center gap-1 rounded-xl border border-brand-200 p-0.5">
                        <button @click="view = 'grid'; localStorage.setItem('shop_view', 'grid')"
                                ...
                                class="rounded-lg p-1.5 transition" aria-label="{{ __('Grid view') }}">
                        ...
                        <button @click="view = 'list'; localStorage.setItem('shop_view', 'list')"
                                ...
                                class="rounded-lg p-1.5 transition" aria-label="{{ __('List view') }}">
                ...
                @if ($products->isEmpty())
                    <x-empty-state
                        ...
                        :caption="__('Nothing matches these filters. Clear them or try a different search.')"
                        :cta="['label' => __('Clear Filters'), 'href' => route('shop.index')]" />
                @else
                    <div :class="view === 'list' ? 'space-y-4' : 'grid grid-cols-2 gap-4 sm:grid-cols-3'">
                                    ...
                                    <div class="flex min-w-0 flex-1 flex-col">
                                        ...
                                        <div class="mt-auto flex items-center justify-between">
                                            ...
                                            <form action="{{ route('cart.add') }}" method="POST" class="m-0">
                                                ...
                                                <button class="rounded-full bg-brand-900 px-4 py-1.5 text-xs font-semibold text-white transition hover:bg-brand-800">{{ __('Add to Bag') }}</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
        ...
        <button x-data="{ visible: false }" x-init="window.addEventListener('scroll', () => visible = window.scrollY > 600)" x-show="visible" x-cloak
                ...
                class="fixed bottom-6 start-6 z-40 grid h-11 w-11 place-items-center rounded-full bg-brand-900 text-white shadow-lg transition hover:bg-brand-800 active:scale-95" aria-label="{{ __('Back to top') }}">
