...
@section('title', 'Contact | Racket Club')
...
@section('content')
    ...
    <div class="mx-auto max-w-3xl px-4 py-14 sm:px-6">
        <h1 class="text-2xl font-bold uppercase tracking-wide text-brand-900">GET IN TOUCH</h1>
        <p class="mt-2 text-sm leading-7 text-brand-600">
            The Racket Club team is on hand for questions, order updates and anything else you need.
            Send us a message here — we usually reply within a few hours.
        </p>
        ...
        @if ($bot)
            ...
            <p class="mt-2 text-center text-xs text-brand-400">Tap the button, then press "Start" in Telegram to begin.</p>
        @endif

        {{-- Fallback channels. Customers who don't use Telegram still have a way through. --}}
        @if ($waLink || $phone || $email || $ig)
            <div class="mt-10">
                <p class="mb-3 text-center text-xs text-brand-400">Or reach us through any of these:</p>
        ...
        @if ($address)
            <div class="mt-10 rounded-card bg-brand-50 p-5 text-center text-sm text-brand-700">
                <span class="text-xs text-brand-400">Address:</span>
