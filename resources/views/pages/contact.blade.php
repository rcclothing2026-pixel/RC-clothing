...
@section('title', 'تماس با ما | چیاکو')
...
@section('content')
    ...
    <div class="mx-auto max-w-3xl px-4 py-14 sm:px-6">
        <h1 class="text-2xl font-bold text-brand-900">تماس با ما</h1>
        <p class="mt-2 text-sm leading-7 text-brand-600">
            تیم پشتیبانی چیاکو در تلگرام آمادهٔ پاسخگویی به سوالات، پیگیری سفارش و راهنمایی شماست.
            پیامتان را همین‌جا بفرستید — معمولاً ظرف چند ساعت پاسخ می‌دهیم.
        </p>
        ...
        @if ($bot)
            <a href="https://t.me/{{ $bot }}?start=contact" target="_blank" rel="noopener"
               class="mt-8 flex items-center justify-center gap-2 rounded-2xl bg-[#229ED9] px-6 py-4 text-base font-bold text-white shadow-sm transition hover:bg-[#1f8fc4]">
                ...
                <span>گفتگو با پشتیبانی در تلگرام</span>
            </a>
            <p class="mt-2 text-center text-xs text-brand-400">برای شروع، روی دکمه بزنید و در تلگرام «Start» را لمس کنید.</p>
        @endif

        {{-- Fallback channels. Customers who don't use Telegram still have a way through. --}}
        @if ($waLink || $phone || $email || $ig)
            <div class="mt-10">
                <p class="mb-3 text-center text-xs text-brand-400">یا از این راه‌ها در تماس باشید:</p>
        ...
        @if ($address)
            <div class="mt-10 rounded-card bg-brand-50 p-5 text-center text-sm text-brand-700">
                <span class="text-xs text-brand-400">آدرس:</span>
