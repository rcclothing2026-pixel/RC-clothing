...
<section class="mx-auto {{ $containerClass }} {{ $padClass }} px-4 sm:px-6">
    <div class="rounded-card bg-brand-900 px-6 py-12 text-center">
        <h2 class="text-2xl font-bold text-white">{{ ($data['heading'] ?? null) ?: __('Join the Club') }}</h2>
        @if (!empty($data['text']))
        ...
        @if ($channel === 'email')
            <form action="{{ route('subscriber.store') }}" method="POST" class="mx-auto mt-6 flex max-w-sm gap-2">
                ...
                <button class="rounded-full bg-accent-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-accent-700">{{ __('Subscribe') }}</button>
            </form>
        @elseif ($tg = ($site['site.telegram'] ?? null))
            ...
            <a href="https://t.me/{{ ltrim($tg, '@') }}" target="_blank" rel="noopener"
               class="mt-6 inline-block rounded-full bg-white px-7 py-3 text-sm font-semibold text-brand-900 transition hover:bg-brand-100">{{ __('Join our Telegram channel') }}</a>
        @endif
    </div>
</section>
