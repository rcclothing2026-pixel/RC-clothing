...
@section('title', __('Account') . ' | Racket Club')

@section('content')
    <div class="mx-auto max-w-5xl px-4 py-10 sm:px-6">
        <div class="mb-8 flex items-center justify-between">
            <h1 class="text-2xl font-bold uppercase tracking-wide text-brand-900">{{ __('ACCOUNT') }}</h1>
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button class="text-sm text-brand-500 transition hover:text-red-500">{{ __('Sign Out') }}</button>
            </form>
        </div>

        <div class="mb-6 flex flex-wrap gap-3">
            <a href="{{ route('wishlist.index') }}" class="rounded-lg border border-brand-200 px-4 py-2 text-sm text-brand-700 hover:bg-brand-50">{{ __('Wishlist') }}</a>
            @if ($user->is_admin)
                <a href="{{ route('admin.dashboard') }}" class="rounded-lg bg-brand-900 px-4 py-2 text-sm font-semibold text-white">{{ __('Admin Panel') }}</a>
            @endif
        </div>
        ...
        @if ($crm && ($crm['ok'] ?? false))
            ...
            <section class="mb-6 rounded-card bg-brand-900 p-6 text-white">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <p class="text-xs text-white/70">{{ __('Club Points') }}</p>
                        <p class="mt-1 text-3xl font-bold fa-num">{{ $club['loyalty_points'] ?? 0 }}</p>
                        @if (!empty($crm['loyalty']['points_value_toman']))
                            <p class="mt-1 text-xs text-white/70 fa-num">{{ __('Worth') }} {{ \App\Support\Money::toman($crm['loyalty']['points_value_toman']) }}</p>
                        @endif
                    </div>
                    <div class="flex gap-6 text-center">
                        <div>
                            <p class="text-2xl font-bold fa-num">{{ $life['orders'] ?? 0 }}</p>
                            <p class="text-xs text-white/70">{{ __('Orders') }}</p>
                        </div>
                        <div>
                            <p class="text-lg font-bold fa-num">{{ \App\Support\Money::toman((int) ($life['total_spent'] ?? 0)) }}</p>
                            <p class="text-xs text-white/70">{{ __('Total Spent') }}</p>
                        </div>
                    </div>
                </div>
                @if (!empty($crm['recent_purchases']))
                    <div class="mt-5 border-t border-white/15 pt-4">
                        <p class="mb-2 text-xs text-white/70">{{ __('Recent purchases (in-store & online)') }}</p>
        ...
        <div x-data="{ tab: 'profile' }">
            ...
            <section x-show="tab === 'profile'" x-cloak
                ...
                <h2 class="mb-4 text-base font-bold text-brand-900">{{ __('Your Details') }}</h2>
                <form action="{{ route('account.profile') }}" method="POST" class="space-y-3">
                    @csrf @method('PATCH')
                    <div>
                        <label class="mb-1 block text-xs text-brand-500">{{ __('Full Name') }}</label>
                    ...
                    <div>
                        <label class="mb-1 block text-xs text-brand-500">{{ __('Mobile') }}</label>
                    ...
                    <div>
                        <label class="mb-1 block text-xs text-brand-500">{{ __('Email (optional)') }}</label>
                    ...
                    <button class="rounded-lg bg-brand-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-800">{{ __('Save') }}</button>
                </form>
                ...
                <div class="mt-6 border-t border-brand-100 pt-5">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            @if ($telegramChat)
                                <p class="mt-1 text-xs text-green-600">{{ __('Your account is connected to Telegram — order updates arrive there.') }}</p>
                            @else
                                <p class="mt-1 text-xs leading-6 text-brand-500">{{ __('Connect Telegram to receive order confirmations and shipping updates straight to your chat.') }}</p>
                            @endif
                        </div>
                        @if ($telegramChat)
                            <form action="{{ route('account.telegram.disconnect') }}" method="POST">
                                @csrf
                                <button class="shrink-0 rounded-lg border border-brand-200 px-4 py-2 text-xs font-medium text-brand-600 hover:bg-brand-50">{{ __('Disconnect') }}</button>
                            </form>
                        @elseif ($telegramReady)
                            <a href="{{ route('account.telegram.connect') }}"
                               class="shrink-0 rounded-lg bg-[#229ED9] px-4 py-2 text-xs font-semibold text-white hover:bg-[#1c8ec2]">{{ __('Connect Telegram') }}</a>
                        @else
                            <span class="shrink-0 self-center text-xs text-brand-300">{{ __('Coming soon') }}</span>
                        @endif
                    </div>
                </div>
            </section>
            ...
            <section x-show="tab === 'addresses'" x-cloak
                     class="rounded-card bg-white p-6 ring-1 ring-brand-100">
                <h2 class="mb-4 text-base font-bold text-brand-900">{{ __('Addresses') }}</h2>
                <div class="space-y-2">
                    @forelse ($addresses as $address)
                        <div class="flex items-start justify-between rounded-lg bg-brand-50 p-3 text-sm">
                            ...
                            <form action="{{ route('account.addresses.destroy', $address) }}" method="POST">
                                @csrf @method('DELETE')
                                <button class="text-xs text-brand-500 hover:text-red-500">{{ __('Remove') }}</button>
                            </form>
                        </div>
                    @empty
                        <p class="text-sm text-brand-500">{{ __("You haven't added an address yet.") }}</p>
                    @endforelse
                </div>

                <details class="mt-4">
                    <summary class="cursor-pointer text-sm font-medium text-accent-600">{{ __('+ Add a new address') }}</summary>
                    <form action="{{ route('account.addresses.store') }}" method="POST" class="mt-3 grid grid-cols-2 gap-2">
                        @csrf
                        <input name="recipient_name" value="{{ old('recipient_name', $user->name) }}" placeholder="{{ __('Recipient name') }}" class="rounded-lg border border-brand-200 px-3 py-2 text-sm" required>
                        <input name="phone" value="{{ old('phone', $user->phone) }}" placeholder="{{ __('Recipient mobile') }}" dir="ltr" class="rounded-lg border border-brand-200 px-3 py-2 text-sm" required>
                        <x-province-select name="province" required />
                        <input name="city" placeholder="{{ __('City') }}" class="rounded-lg border border-brand-200 px-3 py-2 text-sm" required>
                        <input name="postal_code" placeholder="{{ __('Postal code') }}" dir="ltr" class="col-span-2 rounded-lg border border-brand-200 px-3 py-2 text-sm">
                        <textarea name="line" placeholder="{{ __('Full address') }}" class="col-span-2 rounded-lg border border-brand-200 px-3 py-2 text-sm" required></textarea>
                        <label class="col-span-2 flex items-center gap-2 text-xs text-brand-500"><input type="checkbox" name="is_default" value="1"> {{ __('Default address') }}</label>
                        <button class="col-span-2 rounded-lg bg-brand-900 py-2 text-sm font-semibold text-white">{{ __('Save address') }}</button>
                    </form>
                </details>
            </section>
            ...
            <section x-show="tab === 'orders'" x-cloak
                ...
                <div class="mb-4 flex items-center justify-between">
                    <h2 class="text-base font-bold text-brand-900">{{ __('Recent Orders') }}</h2>
                    <a href="{{ route('account.orders') }}" class="text-sm text-accent-600 hover:underline">{{ __('All orders') }}</a>
                </div>
                ...
                @empty
                    <p class="text-sm text-brand-500">{{ __("You haven't placed an order yet.") }}</p>
                @endforelse
            </section>
        </div>
    </div>
