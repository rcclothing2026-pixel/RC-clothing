...
@section('title', 'جزئیات سفارش | چیاکو')
...
@section('content')
    <div class="mx-auto max-w-3xl px-4 py-10 sm:px-6">
        <div class="flex items-center justify-between">
            <a href="{{ route('account.orders') }}" class="text-sm text-accent-600 hover:underline">← بازگشت به سفارش‌ها</a>
            <div class="flex gap-2">
                @if ($order->isPaid())
                    <a href="{{ route('account.orders.return', $order) }}" class="rounded-lg border border-brand-200 px-3 py-1.5 text-sm text-brand-700 transition hover:bg-brand-50">↩ درخواست مرجوعی</a>
                @endif
                <a href="{{ route('account.orders.invoice', $order) }}" target="_blank" class="rounded-lg bg-brand-100 px-3 py-1.5 text-sm font-medium text-brand-700 transition hover:bg-brand-200">🖨 فاکتور</a>
            </div>
        </div>
        ...
        <div class="mt-4 rounded-card bg-white p-6 ring-1 ring-brand-100">
                    ...
                    <div class="flex items-center justify-between py-3 text-sm">
                        <div>
                            ...
                            <p class="mt-0.5 text-xs text-brand-400">
                                ...
                                — تعداد: <span class="fa-num">{{ \App\Support\Money::toPersianDigits((string) $item->quantity) }}</span>
                            </p>
                        </div>
            ...
            <div class="space-y-2 border-t border-brand-100 pt-4 text-sm">
                ...
                <div class="flex justify-between"><span class="text-brand-500">هزینه ارسال ({{ $order->shipping_method_name }})</span><span>{{ $order->shipping_cost_on_delivery ? 'پس‌کرایه' : ($order->shipping_cost > 0 ? \App\Support\Money::toman($order->shipping_cost) : 'رایگان') }}</span></div>
                @if ($order->gift_wrap)
                    <div class="flex justify-between"><span class="text-brand-500">🎁 بسته‌بندی کادویی</span><span>{{ $order->gift_wrap_price > 0 ? \App\Support\Money::toman($order->gift_wrap_price) : 'رایگان' }}</span></div>
                @endif
                <div class="flex justify-between border-t border-brand-100 pt-2 text-base font-bold"><span>مبلغ کل</span><span class="text-brand-900">{{ $order->formattedTotal() }}</span></div>
            </div>
            ...
            @if ($order->gift_wrap && filled($order->gift_message))
                <div class="mt-4 rounded-lg bg-amber-50 p-4 text-sm text-amber-800 ring-1 ring-amber-200">
                    <p class="mb-1 text-xs font-semibold text-amber-700">🎁 متن کارت پیام</p>
            ...
            @if ($order->fulfillmentLocationLabel())
                <div class="mt-4 flex justify-between text-sm">
                    <span class="text-brand-500">تأمین از انبار</span>
            ...
            @if ($order->shipping_address)
                <div class="mt-5 rounded-lg bg-brand-50 p-4 text-xs text-brand-600">
                    <p class="mb-1 font-semibold text-brand-800">آدرس تحویل</p>
